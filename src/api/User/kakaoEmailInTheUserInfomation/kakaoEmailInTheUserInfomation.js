...
export default {
  Mutation: {
    kakaoEmailInTheUserInfomation: async (_, args) => {
      const { email } = args;
      ...
      if (exist) {
        if (exist.inflow === "weberyday") {
          throw Error(
            "해당 kakao의 Email은 다른 계정 로그인용 이메일입니다.",
            "카카오로그인을 제외한 다른 로그인 기능을 사용해주시길 바랍니다."
          );
        } else if (exist.inflow === "kakao") {
          //카카오 로그인을 시켜줘야함 즉 token을 생성해야함
