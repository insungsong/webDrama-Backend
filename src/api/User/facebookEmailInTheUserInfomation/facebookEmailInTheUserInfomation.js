...
export default {
  Mutation: {
    facebookEmailInTheUserInfomation: async (_, args) => {
      const { email } = args;
      const exist = await prisma.user({ email });
      const inflow = "facebook";
      ...
      if (exist) {
        if (exist.inflow === "weberyday") {
          throw Error(
            "해당 Facebook의 Email은 다른 계정 로그인용 이메일입니다.",
            "페이스북 로그인을 제외한 다른 로그인 기능을 사용해주시길 바랍니다."
          );
        } else if (exist.inflow === "facebook") {
          //페이스북 로그인을 시켜줘야함 즉 token을 생성해야함
